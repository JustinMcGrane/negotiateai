...
export const metadata: Metadata = {
  title: 'Operations Manager Resume — AI Builder & Templates | Hayven',
  description: 'Build an operations manager resume that gets interviews. AI-powered ATS scoring, bullet rewrites, and ops-specific keyword analysis. Free to start.',
  alternates: { canonical: 'https://gethayven.com/resume-builder/operations-manager' },
  openGraph: { title: 'Operations Manager Resume Builder | Hayven', description: 'AI resume builder for operations managers. ATS scoring, bullet rewrites, and ops templates.' },
}

const bullets = {
  before: 'Improved operational processes and helped the team become more efficient.',
  after: 'Redesigned the fulfillment workflow for a 300-person warehouse — reducing order processing time by 41%, cutting labor costs by $680K annually, and achieving 99.2% on-time shipment rate across 4 distribution centers.',
}

const tips = [
  { title: 'Cost savings and efficiency are your currency', desc: 'Operations resumes live and die by numbers. Cost reduced, time saved, error rate lowered, throughput increased — if you improved a process, you have a metric. Find it and use it.' },
  { title: 'Scale defines your level', desc: 'Team size, facility square footage, budget managed, SKU count, orders per day — context matters. An ops manager running a 20-person team and one running a 500-person operation are different jobs.' },
  { title: 'Process improvement methodology', desc: 'Lean, Six Sigma, Kaizen, 5S, DMAIC — if you\'ve applied a methodology, name it. Certifications (Green Belt, Black Belt) belong in both your Skills section and your education.' },
  { title: 'Technology and systems signal modernity', desc: 'ERP systems (SAP, Oracle, NetSuite), WMS, TMS, Salesforce Ops — name the systems you\'ve managed. Ops roles vary widely in their tech stack and ATS filters for this.' },
  { title: 'Vendor and supplier management', desc: 'If you\'ve negotiated contracts, managed SLAs, or onboarded/offboarded vendors, say so. Procurement and vendor management are distinct skills that command a premium in ops roles.' },
  { title: 'Cross-functional leadership unlocks senior roles', desc: 'The jump from manager to director often hinges on cross-functional scope. If you\'ve partnered with finance, sales, or product to drive operational changes, make that collaboration explicit.' },
]

const keywords = ['Process Improvement', 'Lean', 'Six Sigma', 'P&L Management', 'Supply Chain', 'ERP', 'SAP', 'Vendor Management', 'KPI', 'SLA', 'Capacity Planning', 'Workforce Management', 'Inventory Management', 'Cost Reduction', 'Operational Excellence']

export default function OperationsManagerResumePage() {
  return (
    <div style={{ background: '#fff', minHeight: '100vh' }}>
      ...
      <section style={{ maxWidth: 860, margin: '0 auto', padding: '56px 24px 64px', textAlign: 'center' }}>
        <div style={{ display: 'inline-block', fontSize: 12, fontWeight: 700, color: '#4169E1', background: '#eff6ff', padding: '4px 14px', borderRadius: 20, marginBottom: 20, letterSpacing: '0.06em' }}>OPERATIONS MANAGER RESUME</div>
        <h1 style={{ fontSize: 'clamp(32px, 5vw, 58px)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: 20, color: '#0f172a' }}>
          An operations manager resume<br />built around measurable results
        </h1>
        <p style={{ fontSize: 17, color: '#475569', lineHeight: 1.75, maxWidth: 560, margin: '0 auto 36px' }}>
          Get ATS scoring, ops-specific bullet rewrites, and keyword gap analysis for your target role — in under 60 seconds. Free to start.
        </p>
      ...
      <section style={{ background: '#f8fafc', borderTop: '0.5px solid #e5e7eb', borderBottom: '0.5px solid #e5e7eb', padding: '64px 24px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '0.5px solid #e5e7eb', overflow: 'hidden' }}>
            <div style={{ padding: '10px 24px', background: '#f8fafc', borderBottom: '0.5px solid #e5e7eb', fontSize: 11, fontWeight: 700, color: '#94a3b8', letterSpacing: '0.06em' }}>OPERATIONS MANAGER · BEFORE & AFTER</div>
      ...
      <section style={{ padding: '64px 24px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(22px, 3vw, 32px)', fontWeight: 800, letterSpacing: '-0.02em', textAlign: 'center', marginBottom: 36, color: '#0f172a' }}>Keywords ATS systems look for in ops resumes</h2>
      ...
      <section style={{ background: '#f8fafc', borderTop: '0.5px solid #e5e7eb', borderBottom: '0.5px solid #e5e7eb', padding: '64px 24px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(22px, 3vw, 32px)', fontWeight: 800, letterSpacing: '-0.02em', textAlign: 'center', marginBottom: 48, color: '#0f172a' }}>How to write an operations manager resume</h2>
      ...
      <section style={{ background: '#0f172a', padding: '72px 24px', textAlign: 'center' }}>
        <h2 style={{ fontSize: 'clamp(24px, 4vw, 38px)', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', marginBottom: 16 }}>Get your ops resume reviewed in 60 seconds</h2>
      ...
      <section style={{ padding: '40px 24px 0', maxWidth: 860, margin: '0 auto' }}>
        <div style={{ borderTop: '0.5px solid #e5e7eb', paddingTop: 32 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
            <Link href="/blog/how-to-ask-for-a-raise-scripts-that-work" style={{ fontSize: 13, fontWeight: 600, color: '#4169E1', textDecoration: 'none', padding: '6px 14px', borderRadius: 20, background: '#eff6ff', border: '0.5px solid #bfdbfe' }}>How to Ask for a Raise</Link>
            <Link href="/raise-request-builder" style={{ fontSize: 13, fontWeight: 600, color: '#059669', textDecoration: 'none', padding: '6px 14px', borderRadius: 20, background: '#ecfdf5', border: '0.5px solid #a7f3d0' }}>Raise Request Builder</Link>
