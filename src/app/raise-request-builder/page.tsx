...
import { ArrowRight, CheckCircle, FileText, TrendingUp, Award } from 'lucide-react'
...
const benefits = [
  { icon: FileText, title: 'A written case, not just a feeling', desc: 'Document your impact, market data, and timeline in a format that holds up in a conversation with your manager.' },
  { icon: TrendingUp, title: 'Market rate benchmarks', desc: 'Know where your current salary sits relative to market — and use that data to anchor your ask.' },
  { icon: Award, title: 'Talking points for the meeting', desc: 'Walk in with specific lines for opening the conversation, handling pushback, and landing on a number.' },
]

const steps = [
  { number: '01', title: 'Tell us about your role and impact', desc: 'Describe what you do, what you\'ve accomplished, and how long it\'s been since your last raise.' },
  { number: '02', title: 'Get your case built', desc: 'We generate a market-benchmarked raise request with your accomplishments framed as business value.' },
  { number: '03', title: 'Have the conversation', desc: 'Use your script to open the conversation and guide it toward the outcome you want.' },
]

export default function RaiseRequestBuilderPage() {
  return (
    <div style={{ background: '#fff', minHeight: '100vh' }}>
      ...
      <section style={{ maxWidth: 900, margin: '0 auto', padding: '56px 24px 72px', textAlign: 'center' }}>
        <h1 style={{ fontSize: 'clamp(36px, 6.5vw, 76px)', fontWeight: 800, lineHeight: 1.12, letterSpacing: '-0.03em', marginBottom: 24, color: '#0f172a', whiteSpace: 'nowrap' }}>
          Make it easy for your<br /><span style={{ color: '#4169E1' }}>manager to say yes.</span>
        </h1>
        <p style={{ fontSize: 18, color: '#475569', lineHeight: 1.7, maxWidth: 560, margin: '0 auto 40px' }}>
          Build a compelling raise request with market data, a documented impact case, and talking points — so you go in prepared, not hopeful.
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 48 }}>
          <Link href="/signup" style={{ height: 48, display: 'inline-flex', alignItems: 'center', gap: 8, padding: '0 28px', background: 'linear-gradient(135deg, #f59e0b 0%, #ef4444 100%)', color: '#fff', borderRadius: 10, fontSize: 15, fontWeight: 700, textDecoration: 'none', boxShadow: '0 3px 10px rgba(239,68,68,0.3)' }}>Try Free Today - No Credit Card Required <ArrowRight size={15} /></Link>
        </div>
        <div style={{ display: 'flex', gap: 24, justifyContent: 'center', flexWrap: 'wrap' }}>
          {['Market-benchmarked ask', 'Impact documentation', 'Free to use'].map(t => (
      ...
      <section style={{ background: '#f8fafc', borderTop: '0.5px solid #e5e7eb', borderBottom: '0.5px solid #e5e7eb', padding: '72px 24px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(22px, 3.5vw, 34px)', fontWeight: 800, letterSpacing: '-0.02em', color: '#0f172a', textAlign: 'center', marginBottom: 12 }}>Your manager won&apos;t give what you don&apos;t ask for.</h2>
          <p style={{ fontSize: 16, color: '#64748b', textAlign: 'center', maxWidth: 520, margin: '0 auto 56px' }}>Most people wait and hope. The ones who get raises build a case and ask.</p>
      ...
      <section style={{ background: '#0f172a', padding: '72px 24px', textAlign: 'center' }}>
        <h2 style={{ fontSize: 'clamp(24px, 4vw, 38px)', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', marginBottom: 16 }}>You&apos;ve earned it. Now ask for it.</h2>
        <p style={{ fontSize: 16, color: '#94a3b8', maxWidth: 480, margin: '0 auto 36px' }}>Build your raise request in minutes — free, no credit card required.</p>
        <Link href="/signup" style={{ height: 50, display: 'inline-flex', alignItems: 'center', gap: 8, padding: '0 32px', background: 'linear-gradient(135deg, #f59e0b 0%, #ef4444 100%)', color: '#fff', borderRadius: 10, fontSize: 15, fontWeight: 700, textDecoration: 'none', boxShadow: '0 3px 10px rgba(239,68,68,0.3)' }}>Try Free Today - No Credit Card Required <ArrowRight size={15} /></Link>
      </section>
